...
This script allows the user to convert a Trello board with customee fields into an Excel report.
...
---------------------------------------------------------------------------------------------------
    ...
    print(
        "ABOUT: This script allows the user to clean and process logs generated by Trello and "
...
def initialisation():
    ...
    print(
        "INSTRUCTION: Before you continue, make sure you have the exported Trello board csv file (trello_board.csv) in the same directory as the "
...
def calulate_default_start_and_end_dates():
    ...
    default_report_end_timestamp = today - datetime.timedelta(idx) + pd.Timedelta(hours=16, seconds=-1)
    default_report_start_timestamp = default_report_end_timestamp - datetime.timedelta(weeks=1) + pd.Timedelta(seconds=1)
...
def specify_report_time_range():
    default_report_start_timestamp, default_report_end_timestamp = calulate_default_start_and_end_dates()
    start_timestamp = input("USER INPUT: Plese press Enter if you want to keep the default start date and time of the report ({}), otherwise, enter the desired start date and time of the report in the following format: yyyy-mm-dd hh:mm\n".format(colored(default_report_start_timestamp, 'green')))
    if not start_timestamp:
        start_timestamp = default_report_start_timestamp
    print("---------------------")
    end_timestamp = input("USER INPUT: Plese press Enter if you want to keep the default end date and time of the report ({}), otherwise, enter the desired end date and time of the report in the following format: yyyy-mm-dd hh:mm\n".format(colored(default_report_end_timestamp, 'green')))
...
def process_timestamps(trello_board, start_timestamp, end_timestamp):
    ...
    trello_board["TICKET_CREATION_TIMESTAMP"] = pd.to_datetime(trello_board["TICKET_CREATION_TIMESTAMP"])
    trello_board["TICKET_RESOLUTION_TIMESTAMP"] = pd.to_datetime(trello_board["TICKET_RESOLUTION_TIMESTAMP"])

    # Generate start and end date of the trello table
    trello_board.start_timestamp = (trello_board["TICKET_CREATION_TIMESTAMP"].min().strftime("%d/%m/%Y|%H:%M:%S"))
    trello_board.end_timestamp = (trello_board["TICKET_CREATION_TIMESTAMP"].max().strftime("%d/%m/%Y|%H:%M:%S"))
    trello_board.no_days = len(trello_board["TICKET_CREATION_TIMESTAMP"].dt.normalize().unique())
    trello_board = trello_board[(trello_board["TICKET_CREATION_TIMESTAMP"] >= start_timestamp)]
    trello_board = trello_board[(trello_board["TICKET_CREATION_TIMESTAMP"] < end_timestamp)]
...
#     trello_board["BUSINSESS_HOURS_TO_ACKNOWLEDGE"] = trello_board.apply(
#         lambda x: len(
#             pd.date_range(
...
#         axis=1,
#     )
#     trello_board["BUSINSESS_HOURS_TO_RESOLVE"] = trello_board.apply(
#         lambda x: len(
#             pd.date_range(
#                 start=x.TICKET_CREATION_TIMESTAMP,
...
#     # Handle labels
#     labels_dict = {'SC' : 'SEC_CONTROL', 'RC' : 'RESOLUTION_CODE', 'CO' : 'DEPENDENT_ON', 'TB' : 'ROOTICKET_CAUSE', 'PR' : 'PRIORITY'}
...
def gen_barplot(trello_board):
    ...
    for required_field in required_fields:
        ...
        required_field_count.to_csv("./OUTPUT/[{}-{}]{}.csv".format(report_start_date_abbriviated,report_end_date_abbriviated,required_field), sep=",")
        fig = go.Figure(
            data=[
                go.Pie(
        ...
        report_start_date_abbriviated = (trello_board["TICKET_CREATION_TIMESTAMP"].min().strftime("%d%b%y")).upper()
        report_end_date_abbriviated = (trello_board["TICKET_CREATION_TIMESTAMP"].max().strftime("%d%b%y")).upper()
        fig.write_image("./OUTPUT/[{}-{}]{}_COUNT.svg".format(report_start_date_abbriviated, report_end_date_abbriviated, required_field))
        print(colored('[SUCCESS]', 'green'), end='.....................')
...
def gen_summary_table(trello_board):
    ...
    summary_table_count.to_excel(
        "./OUTPUT/[{}-{}]SUMMARY_TABLE_COUNT.xlsx".format(report_start_date_abbriviated, report_end_date_abbriviated, duration_start)
    )
    print(colored('[SUCCESS]', 'green'), end='.....................')
...
def gen_trendline(trello_board):
    ...
    fig.update_layout(
        title_text="VSOC tickets trendline grouped by the day<br>Dashed line represents the average no. VSOC tickets ({})".format(
    ...
    report_start_date_abbriviated = (trello_board["TICKET_CREATION_TIMESTAMP"].min().strftime("%d%b%y")).upper()
    report_end_date_abbriviated = (trello_board["TICKET_CREATION_TIMESTAMP"].max().strftime("%d%b%y")).upper()
    fig.write_image("./OUTPUT/[{}-{}]TRENDLINE.svg".format(report_start_date_abbriviated, report_end_date_abbriviated))
    print(colored('[SUCCESS]', 'green'), end='.....................')
...
def gen_internal_report(trello_board):
    ...
    file_name = "[{}-{}]SOC_REPORT.html".format(report_start_date_abbriviated, report_end_date_abbriviated)
    trello_board.to_html("./OUTPUT/{}".format(file_name))
    trello_board.to_excel('./OUTPUT/[{}-{}]INTERNAL_REPORT.xlsx'.format(report_start_date_abbriviated, report_end_date_abbriviated))
